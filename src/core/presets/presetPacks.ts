...
export const presetPacks: PresetPack[] = [
  {
    ...
    category: "foundation",
    style: "bundle",
    description: "Routes China mainland domains and IPs directly using GEOSITE/GEOIP rules (powered by geodata-mode auto-update).",
    supportedTargets: ["openclash", "windows-mihomo", "sparkle"],
    sourceLabel: "Loyalsoldier / MetaCubeX geodata",
    sourceUrl: "https://github.com/Loyalsoldier/v2ray-rules-dat",
    groups: [],
    ruleProviders: [],
    rules: [
      {
        id: "preset-rule-geosite-cn",
        match: { kind: "geosite", value: "cn" },
        policy: { kind: "builtin", value: "DIRECT" },
        priority: 11,
        enabled: true,
        comment: "大陆域名直连（来自 geosite.dat）",
      },
      {
        id: "preset-rule-geoip-cn",
        match: { kind: "geoip", value: "CN" },
        policy: { kind: "builtin", value: "DIRECT" },
        priority: 12,
        enabled: true,
        comment: "大陆 IP 直连（来自 geoip.dat）",
      },
    ],
    i18n: {
      zh: {
        name: "中国大陆直连",
        description: "使用 GEOSITE/GEOIP 规则自动直连大陆域名和 IP（数据随 geodata 自动更新）。",
      },
    },
  },
  {
    ...
    category: "foundation",
    style: "bundle",
    description: "Routes GFW-listed domains through proxy using GEOSITE rule (powered by geodata-mode auto-update).",
    supportedTargets: ["openclash", "windows-mihomo", "sparkle"],
    sourceLabel: "Loyalsoldier / MetaCubeX geodata",
    sourceUrl: "https://github.com/Loyalsoldier/v2ray-rules-dat",
    groups: [],
    ...
    rules: [
      {
        id: "preset-rule-geosite-gfw",
        match: { kind: "geosite", value: "gfw" },
        policy: { kind: "group", value: "Default Proxy" },
        priority: 13,
        enabled: true,
        comment: "GFW 域名走代理（来自 geosite.dat）",
      },
    ],
    i18n: {
      zh: {
        name: "GFW 名单代理",
        description: "把 GFW 列表内的域名自动走代理（数据随 geodata 自动更新）。",
      },
    },
  },
  {
    id: "preset-ad-geosite",
    name: "Ad Block (GeoSite)",
    category: "security",
    style: "bundle",
    description: "Blocks ad domains using GEOSITE category-ads-all (comprehensive, auto-updated).",
    supportedTargets: ["openclash", "windows-mihomo", "sparkle"],
    sourceLabel: "Loyalsoldier / MetaCubeX geodata",
    sourceUrl: "https://github.com/Loyalsoldier/v2ray-rules-dat",
    groups: [],
    ...
    rules: [
      {
        id: "preset-rule-geosite-ads",
        match: { kind: "geosite", value: "category-ads-all" },
        policy: { kind: "builtin", value: "REJECT" },
        priority: 5,
        enabled: true,
        comment: "广告域名拒绝（来自 geosite.dat，包含 EasyList + AdGuard 等）",
      },
    ],
    i18n: {
      zh: {
        name: "广告拦截（GeoSite 版）",
        description: "用 geosite 的完整广告域名库拦截广告（包含 EasyList、AdGuard 等，自动更新）。",
      },
    },
  },
  {
