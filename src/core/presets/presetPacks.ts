    ...
    rules: [
      {
        ...
        match: { kind: "rule_set" as const, value: "geosite:cn" },
        policy: { kind: "builtin" as const, value: "DIRECT" as const },
        priority: 11,
        enabled: true,
        comment: "大陆域名直连（MetaCubeX geosite:cn）",
      },
      {
        id: "preset-rule-geoip-cn",
        match: { kind: "geoip" as const, value: "CN" },
        policy: { kind: "builtin" as const, value: "DIRECT" as const },
        priority: 12,
        enabled: true,
        comment: "大陆 IP 直连（GEOIP）",
      },
    ],
    i18n: {
      zh: {
        name: "中国大陆直连",
        description: "通过 MetaCubeX geosite:cn 规则集，让大陆域名和 IP 走直连。",
      },
    },
  },
  {
